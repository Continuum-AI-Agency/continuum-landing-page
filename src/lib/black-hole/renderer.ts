...
// Adapted from vgpu's optimized-black-hole example (MIT, github.com/vercel-labs/vgpu):
// always centered, DPR-scaled surface, onError hook for the poster fallback.
...
export function createRenderer({ canvas, onError }: RendererOptions) {
  ...
  const initialize = async () => {
    ...
    gpu = nextGpu;
    api = vgpu;
    surface = vgpu.surface(gpu, canvas, { dpr: [1, 2] });
